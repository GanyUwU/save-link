...
import { Eye, EyeOff, Mail, Lock, User, Link2 } from "lucide-react";
    ...
    <div className={`min-h-screen flex items-center justify-center p-4 relative font-sans ${
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <Card className={`p-8 backdrop-blur-lg border-0 shadow-2xl ${
          dark ? 'bg-[#1a1a1a]/90 shadow-black/40' : 'bg-white/80 shadow-blue-500/20'
        }`}>
          <div className="text-center mb-8">
            <div className={`inline-flex items-center justify-center w-16 h-16 rounded-full mb-4 ${
              dark ? 'bg-gradient-to-r from-gray-700 to-gray-600' : 'bg-gradient-to-r from-blue-500 to-indigo-500'
            }`}>
              <User className="text-white" size={24} />
            </div>
            <h2 className={`text-3xl font-bold mb-2 ${dark ? 'text-gray-900' : 'text-gray-800'}`}>
              Create Account
            </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-4">
              <div className="relative">
                <Label htmlFor="email" className={`block text-sm font-medium mb-2 ${dark ? 'text-gray-200' : 'text-gray-700'}`}>
                  Email Address
                </Label>
                <div className="relative">
                  <Mail className={`absolute left-3 top-1/2 transform -translate-y-1/2 ${dark ? 'text-gray-400' : 'text-gray-500'}`} size={18} />
                  <Input
                    id="email"
                    type="email"
                    value={email}
              ...
              <div className="relative">
                <Label htmlFor="password" className={`block text-sm font-medium mb-2 ${dark ? 'text-gray-200' : 'text-gray-700'}`}>
                  Password
                </Label>
                <div className="relative">
                  <Lock className={`absolute left-3 top-1/2 transform -translate-y-1/2 ${dark ? 'text-gray-400' : 'text-gray-500'}`} size={18} />
                  <Input
                    id="password"
                  ...
                  <button
                    ...
                    className={`absolute right-3 top-1/2 transform -translate-y-1/2 ${dark ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-gray-700'} transition-colors`}
                  >
            ...
            <Button
              type="submit"
              className={`w-full h-12 text-gray-800 font-medium text-xl rounded-lg transition-all duration-200 ${
                dark
          ...
          <div className="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
            <p className={`text-sm text-center ${dark ? 'text-blue-700' : 'text-gray-600'}`}>
              Already have an account?{" "}
              <Link
                to="/login"
                className={`font-medium transition-colors ${dark ? 'text-gray-900 hover:text-white' : 'text-blue-600 hover:text-blue-500'} hover:underline`}
              >
                Sign in
              </Link>
            </p>
